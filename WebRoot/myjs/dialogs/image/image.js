...
(function() {
    ...
    dialog.onok = function() {
      ...
      switch (id) {
        ...
        list = uploadImage.getInsertList();
          ...
          break;
        case 'online':
        list = onlineImage.getInsertList();//add by lvbin@2017-10-19 多图上传的在线管理功能，获取要插入到ueditor的图片的list
          break;
        case 'search':
        console.log("search!")        
  ...
  UploadImage.prototype = {
    ...
    initUploader: function() {
      ...
      uploader = _this.uploader = WebUploader.create({
        ...
        duplicate: true,
        fileSingleSizeLimit: imageMaxSize, // 默认 1 M
  ...
  OnlineImage.prototype = {
    ...
    pushData: function(list) {
      ...
      var httpurl = "https://" + window.location.host;
      for (i = 0; i < list.length; i++) {
        if (list[i] && list[i].url) {
          ...
          img.setAttribute('src', httpurl + urlPrefix + list[i].url + (list[i].url.indexOf('?') == -1 ? '?noCache=' : '&noCache=') + (+new Date()).toString(36));
          img.setAttribute('_src', httpurl + urlPrefix + list[i].url);
    ...
    scale: function(img, w, h, type) {
      ...
      if (ow >= oh) //如果长大于等于宽
      {
        ...
        { //按照长同比例缩小
          img.width = tmp - 1;
          img.height = oh * (tmp - 1) / ow;
          var margtintop = (160 - img.height) / 2;
          img.setAttribute('style', 'margin-top:' + margtintop + 'px');
        }
      } else {
        if (oh >= tmp) //如果宽大于容器宽
        { //按照宽同比例缩小
          img.height = tmp - 1;
          img.width = ow * (tmp - 1) / oh;
        }
      }
      //给图片的style添加max-width属性 add by lvbin@2017-10-19
      var strStyle = "";
      strStyle = img.getAttribute("style");
      if (strStyle.length > 0)
      {
        strStyle += ";max-width:100%;";
        img.setAttribute("style", strStyle);
      }
      else
      {
    ...
    /* add by lvbin@2017-10-19 
      多图上传的在线管理中所选择图片的list函数，添加了style="max-width:100%;"
    */
    getInsertList: function() {
      ...
      for (i = 0; i < lis.length; i++) {
        if (domUtils.hasClass(lis[i], 'selected')) {
          var img = lis[i].firstChild,
            ...
            strStyle = img.getAttribute("style");//add by lvbin@2017-10-19
          list.push({
            src: src,
            _src: src,
            alt: src.substr(src.lastIndexOf('/') + 1),
            floatStyle: align,
            style: strStyle,//add by lvbin@2017-10-19
          });
        }
